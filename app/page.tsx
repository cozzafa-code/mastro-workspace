...
export default function Home() {
  ...
  const WorkspacePanelAI: React.FC<{ utente: string; workspaceData: any }> = ({ utente, workspaceData }) => {
    ...
    // Esegue azione AI sul workspace
    const eseguiAzione = async (action: any): Promise<string> => {
      try {
        if (action.tipo === 'crea_task') {
          await supabase.from('tasks').insert({
            titolo: action.titolo, chi: action.chi || utente,
            stato: 'aperto', priorita: action.priorita || '3',
            scadenza: action.scadenza || null, progetto: action.progetto || null,
          })
          await loadAll()
          return `✅ Task creata: "${action.titolo}"${action.scadenza ? ` · scadenza ${action.scadenza}` : ''}`
        }
        if (action.tipo === 'crea_appuntamento') {
          await supabase.from('calendario_eventi').insert({
            titolo: action.titolo, data: action.data || oggi,
            ora_inizio: action.ora || '09:00', tipo: 'appuntamento',
            descrizione: action.descrizione || null, utente, colore: '#2563EB',
          })
          await loadAll()
          return `✅ Appuntamento creato: "${action.titolo}" il ${action.data || oggi}`
        }
        if (action.tipo === 'crea_delega') {
          await supabase.from('personal_tasks').insert({
            titolo: action.titolo, utente: action.a,
            tipo: 'delega', creato_da: utente, assegnato_a: action.a,
            stato: 'aperto', scadenza: action.scadenza || null,
          })
          await loadAll()
          return `✅ Delega inviata a ${action.a === 'fabio' ? 'Fabio' : 'Lidia'}: "${action.titolo}"`
        }
        if (action.tipo === 'info_fatture') {
          const { data: fatt } = await supabase.from('fatture').select('*').order('created_at', { ascending: false }).limit(10)
          const daIncassare = (fatt || []).filter((f: any) => f.tipo === 'attiva' && f.stato !== 'pagata').reduce((a: number, f: any) => a + Number(f.totale || 0), 0)
          const daPagare = (fatt || []).filter((f: any) => f.tipo === 'passiva' && f.stato !== 'pagata').reduce((a: number, f: any) => a + Number(f.totale || 0), 0)
          const scadute = (fatt || []).filter((f: any) => f.data_scadenza && f.data_scadenza < oggi && f.stato !== 'pagata')
          return `💰 Fatture: Da incassare €${daIncassare.toLocaleString('it-IT')} · Da pagare €${daPagare.toLocaleString('it-IT')}${scadute.length > 0 ? ` · ⚠️ ${scadute.length} scadute` : ''}`
        }
      } catch (e) { return '❌ Errore nell\'eseguire l\'azione.' }
      return ''
    }

    const systemCtx = `Sei MASTRO AI, assistente operativo di ${utente === 'fabio' ? 'Fabio' : 'Lidia'}. Oggi: ${oggi}.

DATI WORKSPACE:
- MRR: €${totMRR}/mo
- Progetti attivi: ${progetti.filter((p: any) => p.stato === 'attivo').map((p: any) => `${p.nome} (€${p.mrr || 0}/mo, ${p.beta_clienti || 0} clienti)`).join(', ') || 'nessuno'}
- Task aperte: ${tasks.slice(0, 8).map((t: any) => `"${t.titolo || t.testo}" [${t.chi || '?'}${t.scadenza ? ', scade ' + t.scadenza : ''}]`).join(', ') || 'nessuna'}
- Fatture: ${fatture.filter((f: any) => f.stato !== 'pagata').length} aperte

Puoi AGIRE sul workspace. Quando l'utente vuole:
- Creare task → rispondi SOLO con JSON: {"azione":{"tipo":"crea_task","titolo":"...","chi":"fabio|lidia","scadenza":"YYYY-MM-DD","priorita":"1-5"}}
- Creare appuntamento → rispondi SOLO con JSON: {"azione":{"tipo":"crea_appuntamento","titolo":"...","data":"YYYY-MM-DD","ora":"HH:MM","descrizione":"..."}}
- Delegare qualcosa → rispondi SOLO con JSON: {"azione":{"tipo":"crea_delega","titolo":"...","a":"fabio|lidia","scadenza":"YYYY-MM-DD"}}
- Info su fatture/pagamenti → rispondi SOLO con JSON: {"azione":{"tipo":"info_fatture"}}
- Per domande informative → rispondi normalmente in italiano, breve e diretto.

Se l'input è vocale e informale, interpretalo bene. Es: "domani mattina alle 9 chiamata con Mario" → appuntamento "Chiamata con Mario" data domani ora 09:00.`

    const sendAI = async (testo?: string) => {
      const q = (testo || aiInput).trim()
      if (!q || aiLoading) return
      setAiInput('')
      setMsgs(m => [...m, { role: 'user', text: q }])
      setAiLoading(true)
      setTimeout(() => bottomAIRef.current?.scrollIntoView({ behavior: 'smooth' }), 50)

      try {
        const openaiKey = process.env.NEXT_PUBLIC_OPENAI_API_KEY
        if (!openaiKey) {
          setMsgs(m => [...m, { role: 'assistant', text: '⚠️ Configura NEXT_PUBLIC_OPENAI_API_KEY su Vercel.' }])
        ...
        try {
          ...
          if (parsed.azione) {
            const risultato = await eseguiAzione(parsed.azione)
            setMsgs(m => [...m, { role: 'assistant', text: risultato, action: parsed.azione.tipo }])
            setAiLoading(false)
            setTimeout(() => bottomAIRef.current?.scrollIntoView({ behavior: 'smooth' }), 100)
            return
          }
        } catch {}
    ...
    // Auto-send dopo input vocale
    React.useEffect(() => {
      if (aiInput && !listening) {
        const timer = setTimeout(() => sendAI(aiInput), 300)
        return () => clearTimeout(timer)
      }
    }, [aiInput, listening])

    const ACCENT_AI = utente === 'fabio' ? '#0A8A7A' : '#BE185D'
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {msgs.length === 0 && (
            <div style={{ padding: '10px 0' }}>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.8 }}>Posso fare per te:</div>
              {[
                { icon: '✓', text: 'Crea task "chiamare Mario domani"' },
                { icon: '📅', text: 'Appuntamento domani alle 10' },
                { icon: '→', text: 'Delega a Lidia: mandare email' },
                { icon: '💰', text: 'Quante fatture ho in sospeso?' },
                { icon: '📊', text: 'Come va il MRR?' },
              ].map(s => (
                <button key={s.text} onClick={() => sendAI(s.text)}
                  style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, marginBottom: 5, color: 'rgba(255,255,255,0.6)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
          ...
          {msgs.map((m, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
              <div style={{
                maxWidth: '90%', padding: '9px 13px', borderRadius: m.role === 'user' ? '12px 12px 2px 12px' : '12px 12px 12px 2px',
                background: m.role === 'user' ? ACCENT_AI : m.action ? 'rgba(15,123,90,0.25)' : 'rgba(255,255,255,0.09)',
                border: m.action ? '1px solid rgba(15,123,90,0.4)' : 'none',
          ...
          {aiLoading && (
            <div style={{ display: 'flex', gap: 4, padding: '4px 0' }}>
              {[0,1,2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: ACCENT_AI, animation: `pulse 1s ${i * 0.2}s ease-in-out infinite` }} />)}
        ...
        <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          {listening && (
            <div style={{ textAlign: 'center', padding: '8px 0', fontSize: 12, color: '#FCA5A5', marginBottom: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#DC4444', animation: 'pulse 1s ease-in-out infinite' }} />
              Sto ascoltando...
            </div>
          )}
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <button onClick={toggleVoice}
              style={{ width: 42, height: 42, borderRadius: '50%', background: listening ? '#DC4444' : 'rgba(255,255,255,0.1)', border: `2px solid ${listening ? '#DC4444' : 'rgba(255,255,255,0.15)'}`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                <path d="M12 1a3 3 0 00-3 3v8a3 3 0 006 0V4a3 3 0 00-3-3z" fill={listening ? '#fff' : 'rgba(255,255,255,0.7)'}/>
                <path d="M19 10v2a7 7 0 01-14 0v-2M12 19v4M8 23h8" stroke={listening ? '#fff' : 'rgba(255,255,255,0.7)'} strokeWidth="2" strokeLinecap="round"/>
              </svg>
            </button>
            <input value={aiInput} onChange={e => setAiInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendAI() } }}
              placeholder={recognition ? 'Scrivi o premi 🎤 per parlare...' : 'Scrivi qui...'}
            ...
            <button onClick={() => sendAI()} disabled={!aiInput.trim() || aiLoading}
              style={{ width: 42, height: 42, borderRadius: '50%', background: aiInput.trim() ? ACCENT_AI : 'rgba(255,255,255,0.07)', border: 'none', cursor: aiInput.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background 0.2s' }}>
